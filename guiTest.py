...
#     dpg.add_slider_float(label="float", default_value=0.273, max_value=1)
#
# dpg.create_viewport(title='Dalley Custom Title', width=600, height=200)
# dpg.setup_dearpygui()
# dpg.show_viewport()
...
# dpg.destroy_context()
#
# 3.1.5 dalley attempt to create primary window with created items
...
# print(dpg.get_item_label("Btn2"))
#
# dpg.create_viewport(title='Dalley Initial Button Primary test', width=600, height=200)
# dpg.setup_dearpygui()
# dpg.show_viewport()
# dpg.set_primary_window("Primary Window", True)
# dpg.start_dearpygui()
...
# 3.3 configuration, state, and info
...
#attempt to create interface for nuzlocke tracker

import dearpygui.dearpygui as dpg
import main
from main import new_game, load_game

dpg.create_context()

with dpg.window(tag="Primary Window"):
    dpg.add_button(label="New Run",callback= new_game())
    dpg.add_button(label='Load Run',callback= load_game())
