...
PREVIEW_DIR = ROOT / "diamondpearl_build5_preview"
VERSION_GROUP_ID = 8
LOAD_BUILD = 5
...
CREATE TEMP TABLE diamondpearl_trainer_location_stage (
  encounter_name text,
...
\\copy diamondpearl_trainer_location_stage FROM '{stage_csv.as_posix()}' WITH (FORMAT csv, HEADER true)

SELECT 'stage_rows' AS metric, count(*) AS value FROM diamondpearl_trainer_location_stage;
SELECT 'stage_located_rows' AS metric, count(*) AS value FROM diamondpearl_trainer_location_stage WHERE nullif(canonical_location_id, '') IS NOT NULL;
...
def main():
    parser = argparse.ArgumentParser(description="Update loaded Diamond/Pearl trainer locations from the generated preview.")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        stage_csv = tmp_path / "diamondpearl_trainer_location_stage.csv"
        sql_file = tmp_path / "update_diamondpearl_trainer_locations.sql"
        count = write_stage_csv(stage_csv, Path(args.preview_dir))
        print(f"Prepared {count} Diamond/Pearl trainer location stage rows.")
