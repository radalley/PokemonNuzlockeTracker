...
def get_games(conn):
    games = conn.execute(f'SELECT game_id, name, game_tag, generation, version_group_id from games where valid_game = "valid"').fetchall()
    return games

def _get_game_generation(conn, game_id=None, run_id=None):
    if game_id is not None:
        row = conn.execute('select generation from games where game_id = ?', (game_id,)).fetchone()
        return row['generation'] if row else None
    if run_id is not None:
        row = conn.execute(
            'select g.generation from runs r join games g on r.game_id = g.game_id where r.run_id = ?',
            (run_id,)
        ).fetchone()
        return row['generation'] if row else None
    return None
...
def _build_generation_patch_join(table_name, join_alias, species_expr, generation_expr):
    return (
        f'left join {table_name} {join_alias} on {join_alias}.rowid = ('
        f'  select x.rowid from {table_name} x '
        f'  where x.species_id = {species_expr} '
        f'    and (x.generation is null or x.generation >= ifnull({generation_expr}, 9999)) '
...
def set_active_game(conn, game):
    ...
    state['version_group_id'] = conn.execute('select version_group_id from games where game_id = (?)', (game,)).fetchone()['version_group_id']

def create_run(conn, name, user_id=None):
    _ensure_auth_schema(conn)
    conn.execute(
        "insert into runs (game_id, name, user_id) values (?,?,?)",
    ...
    for row in runs:
        ...
        if latest_attempt is not None:
            ...
            attempt_row = conn.execute(
                'select attempt_id from attempts where run_id = ? and attempt_number = ?',
...
def get_run_by_id(conn, run_id, attempt_number, user_id=None):
    ...
    query = (
        ...
        '  from runs left join games on runs.game_id = games.game_id where runs.run_id = ?'
        ') as run '
        'left join attempts on run.run_id = attempts.run_id '
        'where attempts.attempt_number = ?'
    )
    params = [run_id, attempt_number]
    if user_id is not None:
        query += ' and run.user_id = ?'
        params.append(user_id)
    return conn.execute(query, params).fetchone()

def delete_run(conn, run_id):
    attempts = [x['attempt_id'] for x in conn.execute('select attempt_id from attempts where run_id = (?)',(run_id,)).fetchall()]
    #delete from runs
    conn.execute('delete from runs where run_id = (?)',(run_id,))
    #delete from pokebank
    conn.execute('delete from pokebank where run_id = (?)',(run_id,))
    #delete from party
    for attempt_id in attempts:
        conn.execute('delete from party where attempt_id = (?)',(attempt_id,))
        # delete from attempts
        conn.execute('delete from attempts where attempt_id = (?)',(attempt_id,))
        conn.execute('delete from badges where attempt_id = (?)',(attempt_id,))
        conn.execute('delete from trainers_defeated where attempt_id = (?)',(attempt_id,))
    conn.commit()
...
def new_attempt(conn, ):
    ...
    if len(attempts) == 0:
        ...
        if has_attempt_badges_earned:
            conn.execute("insert into attempts (run_id, attempt_number, starter, badges_earned) values (?,?,?,?)",
...
def get_attempts_for_run(conn, run_id):
    return conn.execute(
        'select attempt_number from attempts where run_id = ? order by attempt_number asc',
        (run_id,)
    ).fetchall()

def update_starter(conn, run_id, attempt_number, starter):
    conn.execute(
        'update attempts set starter = ? where run_id = ? and attempt_number = ?',
...
def create_attempt_for_run(conn, run_id):
    latest = conn.execute(
        'select max(attempt_number) as max_num from attempts where run_id = ?',
        (run_id,)
    ).fetchone()['max_num'] or 0
    new_num = latest + 1
    if _has_column(conn, 'attempts', 'badges_earned'):
        conn.execute(
            'insert into attempts (run_id, attempt_number, starter, badges_earned) values (?, ?, ?, ?)',
            (run_id, new_num, 'Fire', '')
        )
    else:
        conn.execute(
            'insert into attempts (run_id, attempt_number, starter) values (?, ?, ?)',
            (run_id, new_num, 'Fire')
        )
    conn.commit()
    return new_num
...
def get_species_summary(conn, species_id, game_id=None):
    ...
    row = conn.execute(
        ...
        'from species s '
        + _build_generation_patch_join('species_stats', 'ss', 's.species_id', '?')
        + _build_generation_patch_join('species_types', 'st', 's.species_id', '?')
        + 'where s.species_id = ?',
...
def _get_run_version_group_id(conn, run_id):
    row = conn.execute(
        ...
        'from runs r join games g on r.game_id = g.game_id '
        'where r.run_id = ?',
        (run_id,)
    ).fetchone()
...
def _ensure_bonus_locations_schema(conn):
    table_row = conn.execute(
        "select sql from sqlite_master where type = 'table' and name = 'bonus_locations'"
    ).fetchone()
    if not table_row:
        conn.execute(
            'create table bonus_locations ('
            'bonus_location_id integer primary key autoincrement, '
            'run_id integer, '
            'attempt_id integer, '
    ...
    columns = conn.execute("pragma table_info('bonus_locations')").fetchall()
    canonical_col = next((c for c in columns if c['name'] == 'canonical_location_id'), None)
    secondary_col = next((c for c in columns if c['name'] == 'secondary_sort_order'), None)
    needs_migration = False

    if canonical_col and canonical_col['pk']:
        needs_migration = True
    if secondary_col and str(secondary_col['dflt_value'] or '').strip("'") == 'kanto':
        needs_migration = True

    if not needs_migration:
        return

    conn.execute('alter table bonus_locations rename to bonus_locations_old')
    conn.execute(
        'create table bonus_locations ('
        'bonus_location_id integer primary key autoincrement, '
        'run_id integer, '
    ...
    conn.execute(
        ...
        ') '
        'select run_id, attempt_id, canonical_location_id, canonical_name, sort_order, '
        'case '
        "  when typeof(secondary_sort_order) = 'integer' then secondary_sort_order "
        "  when trim(coalesce(secondary_sort_order, '')) glob '-?[0-9]*' then cast(secondary_sort_order as integer) "
        '  else 0 '
        'end, '
        'coalesce(is_active, 1), version_group_id, event_type '
        'from bonus_locations_old'
    )
    conn.execute('drop table bonus_locations_old')
    conn.commit()
...
def create_bonus_location(conn, run_id, attempt_number, canonical_location_id):
    ...
    base_row = conn.execute(
        ...
        'where canonical_location_id = ? and version_group_id = ? '
        'limit 1',
    ...
    existing_max = conn.execute(
        ...
        'where run_id = ? and attempt_id = ? and canonical_location_id = ? and is_active = 1',
...
def delete_bonus_location(conn, run_id, attempt_number, canonical_location_id, secondary_sort_order):
    ...
    bonus_row = conn.execute(
        ...
        'where run_id = ? and attempt_id = ? and canonical_location_id = ? and secondary_sort_order = ? and is_active = 1 '
        'limit 1',
    ...
    pokemon_ids = [
        row['pokemon_id'] for row in conn.execute(
            ...
            'where run_id = ? and attempt_id = ? and location_id = ? and coalesce(bonus_location, 0) = ?',
            (run_id, attempt_id, canonical_location_id, secondary_sort_order)
        ).fetchall()
    ]
    ...
    if pokemon_ids:
        ...
        conn.execute(
            f'delete from party where attempt_id = ? and pokemon_id in ({placeholders})',
    ...
    conn.execute(
        ...
        'where run_id = ? and attempt_id = ? and canonical_location_id = ? and secondary_sort_order = ?',
        (run_id, attempt_id, canonical_location_id, secondary_sort_order)
    )
    conn.commit()
...
def get_party_for_attempt(conn, run_id, attempt_number):
    ...
    has_badges_earned = conn.execute(
        "select 1 from pragma_table_info('pokebank') where name = 'badges_earned' limit 1"
    ...
    return conn.execute(
        ...
        + _build_generation_patch_join('species_stats', 'ss', 'pb.species_id', '?')
        + _build_generation_patch_join('species_types', 'st', 'pb.species_id', '?')
        + _build_generation_patch_join('species_abilities', 'sa', 'pb.species_id', '?')
        + 'where p.attempt_id = ? '
...
def add_to_party_for_attempt(conn, run_id, attempt_number, pokemon_id):
    ...
    occupied = {r['party_slot'] for r in conn.execute(
        'select party_slot from party where attempt_id = ?', (attempt_id,)
    ).fetchall()}
    # check if already in party
    already = conn.execute(
        'select party_slot from party where attempt_id = ? and pokemon_id = ?', (attempt_id, pokemon_id)
    ).fetchone()
    if already:
    ...
    if next_slot is None:
        return None
    conn.execute(
        'insert into party (attempt_id, party_slot, pokemon_id) values (?, ?, ?)',
...
def remove_from_party_for_attempt(conn, run_id, attempt_number, pokemon_id):
    ...
    attempt_id = row['attempt_id']
    conn.execute(
        'delete from party where attempt_id = ? and pokemon_id = ?',
...
def _has_column(conn, table_name, column_name):
    return conn.execute(
        f"select 1 from pragma_table_info('{table_name}') where name = ? limit 1",
        (column_name,)
    ).fetchone() is not None
...
def get_user_by_email(conn, email):
    ...
    if not normalized_email:
        return None
    row = conn.execute(
        'select user_id, email, display_name, created_at from users where lower(email) = ?',
        (normalized_email,)
    ).fetchone()
    return _public_user(row)
...
def authenticate_user(conn, email, password):
    ...
    normalized_email = _normalize_email(email)
    row = conn.execute(
        'select user_id, email, password_hash, display_name, created_at from users where lower(email) = ?',
        (normalized_email,)
    ).fetchone()
...
def claim_legacy_runs_for_user(conn, user_id):
    ...
    if int(user_count or 0) != 1:
        return 0
    updated = conn.execute('update runs set user_id = ? where user_id is null', (user_id,))
    conn.commit()
    return updated.rowcount

def get_run_owner_id(conn, run_id):
    _ensure_auth_schema(conn)
    row = conn.execute('select user_id from runs where run_id = ?', (run_id,)).fetchone()
...
def get_run_id_for_pokemon(conn, pokemon_id):
    row = conn.execute('select run_id from pokebank where pokemon_id = ?', (pokemon_id,)).fetchone()
...
def _get_attempt_badge_ids(conn, attempt_id, run_id=None, fallback_to_pokebank=False):
    ...
    if _has_column(conn, 'attempts', 'badges_earned'):
        row = conn.execute(
            'select badges_earned from attempts where attempt_id = ?',
            (attempt_id,)
        ).fetchone()
    ...
    if fallback_to_pokebank and run_id is not None and _has_column(conn, 'pokebank', 'badges_earned'):
        badge_rows = conn.execute(
            'select badges_earned from pokebank where run_id = ? and attempt_id = ?',
            (run_id, attempt_id)
        ).fetchall()
        badge_ids = set()
...
def _set_attempt_badge_ids(conn, attempt_id, badge_ids):
    ...
    conn.execute(
        'update attempts set badges_earned = ? where attempt_id = ?',
...
def mark_trainer_victory(conn, run_id, attempt_number, trainer_id, trainer_name, trainer_class, encounter_title):
    ...
    existing = conn.execute(
        'select 1 from trainers_defeated where run_id = ? and attempt_id = ? and trainer_id = ? limit 1',
        (run_id, attempt_id, trainer_id)
    ).fetchone()
    if not existing:
        conn.execute(
            'insert into trainers_defeated (run_id, attempt_id, trainer_id) values (?, ?, ?)',
            (run_id, attempt_id, trainer_id)
        )
    ...
    if has_trainers_defeated:
        ...
        for pr in party_rows:
            ...
            conn.execute(
                'update pokebank set trainers_defeated = ? where pokemon_id = ?',
    ...
    if is_gym_leader and has_badges_earned:
        ...
        if badge_id is not None:
            badge_row = conn.execute(
                'select badge_id, badge_name from badges where badge_id = ? limit 1',
                (badge_id,)
            ).fetchone()
            ...
            for pr in party_rows:
                ...
                conn.execute(
                    'update pokebank set badges_earned = ? where pokemon_id = ?',
...
def get_attempt_page_data(conn, run_id, attempt_number):
    ...
    game_row = conn.execute('select version_group_id from games where game_id = ?', (game_id,)).fetchone() if game_id else None
    ...
    if attempt_row:
        defeated_rows = conn.execute(
            'select trainer_id from trainers_defeated where run_id = ? and attempt_id = ?',
    ...
    if attempt_row and location_ids:
        ...
        trainer_rows = conn.execute(
            ...
            f'count(case when coalesce(tp.is_event, 0) = 0 then 1 end) as trainer_count, '
            f'count(case when coalesce(tp.is_event, 0) = 0 and td.trainer_id is null then 1 end) as available_trainer_count '
            f'from trainer_pool tp '
            f'left join trainers_defeated td '
            f'on td.trainer_id = tp.trainer_id and td.run_id = ? and td.attempt_id = ? '
    ...
    if location_ids and game_id:
        ...
        rows = conn.execute(
            ...
            f'WHERE ep.canonical_location_id IN ({placeholders}) AND ep.game_id = ?',
        ...
        for row in rows:
            ...
            if row[1] in pool_species_ids[lid]:
                continue
            pool_species_ids[lid].add(row[1])
            pools[lid].append({'species_id': row[1], 'name': row[2]})
...
def get_attempt_session_stats(conn, run_id, attempt_number):
    ...
    trainers_defeated = conn.execute(
        'select count(distinct trainer_id) as count from trainers_defeated where run_id = ? and attempt_id = ?',
        (run_id, attempt_id)
    ).fetchone()['count']
...
def upsert_encounter(conn, run_id, attempt_number, location_id, species_id, nickname, nature, status, shiny, pokemon_id=None, bonus_location=0):
    ...
    if pokemon_id:
        conn.execute(
            'update pokebank set species_id=?, location_id=?, nickname=?, nature=?, status=?, shiny=?, bonus_location=? where pokemon_id=?',
        ...
        conn.execute(
            'insert into pokebank (run_id, attempt_id, species_id, location_id, nickname, nature, status, shiny, bonus_location) values (?,?,?,?,?,?,?,?,?)',
...
def delete_encounter(conn, pokemon_id):
    conn.execute('delete from pokebank where pokemon_id = ?', (pokemon_id,))
    conn.commit()
def get_script(conn, starter, version_group_id=None, run_id=None, attempt_number=None):
    ...
    if version_group_id is not None:
        boss_filter = 'and eb.version_group_id = ? '
        loc_filter = 'where version_group_id = ? '
    ...
    if run_id is not None and attempt_number is not None:
        ...
        if attempt_row:
            bonus_sql = (
                'union all '
                'select bl.canonical_location_id as event_id, bl.canonical_name as display_name, '
                'bl.sort_order, coalesce(bl.secondary_sort_order, 0) as secondary_sort_order, '
                'bl.event_type, null, null, null, null, 1 as is_bonus_location '
                'from bonus_locations bl '
                'where bl.run_id = ? and bl.attempt_id = ? and bl.is_active = 1 '
            )
            params.append(run_id)
            params.append(attempt_row['attempt_id'])

    return conn.execute(
        'select eb.trainer_id as event_id, eb.encounter_title as display_name, eb.sort_order, 0 as secondary_sort_order, eb.event_type, tp.encounter_name, tp.trainer_name, tp.trainer_class, tp.trainer_items, 0 as is_bonus_location '
        'from event_bosses eb left join trainer_pool tp on eb.trainer_id = tp.trainer_id '
        'where (eb.starter = (?) or eb.starter is null or eb.starter = "") '
        f'{boss_filter}'
        'union all '
        'select canonical_location_id as event_id, canonical_name as display_name, sort_order, coalesce(secondary_sort_order, 0) as secondary_sort_order, event_type, null, null, null, null, 0 as is_bonus_location '
...
def get_location_by_id(conn, location_id):
    return conn.execute('select canonical_name from event_locations where canonical_location_id = (?) and version_group_id = (?)', (location_id, state['version_group_id'],)).fetchone()[0]
...
def get_pokebank_with_stats(conn, run_id, attempt_number):
    ...
    has_trainers_defeated = conn.execute(
        "select 1 from pragma_table_info('pokebank') where name = 'trainers_defeated' limit 1"
    ).fetchone() is not None
    ...
    rows = conn.execute(
        ...
        f'join runs r on pb.run_id = r.run_id '
        f'join games g on r.game_id = g.game_id '
        f'left join species s on pb.species_id = s.species_id '
        f'left join event_locations el on el.canonical_location_id = pb.location_id and el.version_group_id = g.version_group_id '
        ...
        + f'where pb.run_id = ? and a.attempt_number = ?',
        (run_id, attempt_number)
    ).fetchall()
...
def get_pokebank_for_attempt(conn, run_id, attempt_number):
    ...
    rows = conn.execute(
        ...
        'left join event_locations el on el.canonical_location_id = pb.location_id and el.version_group_id = ? '
        'where pb.run_id = (?) and a.attempt_number = (?)',
...
def get_trainers_by_location(conn, location_id, run_id=None, attempt_number=None):
    ...
    if run_id is not None and attempt_id is not None:
        defeated_join = (
            ...
            'on td.trainer_id = tp.trainer_id and td.run_id = ? and td.attempt_id = ? '
        )
    ...
    return conn.execute(
        ...
        'coalesce(tp.is_event, 0) as is_event, '
        + defeated_select +
        'from trainer_pool tp '
        + defeated_join +
        'where tp.location_id = ? and coalesce(tp.is_rematch, "") != "True" '
        'order by coalesce(tp.is_event, 0) asc, tp.trainer_id asc',
        params
    ).fetchall()
...
def _resolve_move_details(conn, move_id, version_group_id):
    ...
    rows = conn.execute(
        ...
        'from moves where move_id = ? order by version_group_id asc',
        (move_id,)
    ).fetchall()
...
def _pick_moveset_version_group(conn, species_id, target_version_group_id):
    rows = conn.execute(
        'select distinct version_group_id from movesets where species_id = ? and learn_method = "level-up" order by version_group_id asc',
        (species_id,)
    ).fetchall()
    if not rows:
        return None
...
def _resolve_levelup_moves(conn, species_id, level, version_group_id):
    ...
    rows = conn.execute(
        ...
        'where species_id = ? and learn_method = "level-up" and version_group_id = ? and learn_level <= ? '
...
def get_trainer_parties_by_encounter(conn, trainer_name, game_id=None):
    ...
    game_generation = _get_game_generation(conn, game_id=game_id)
    if game_id is not None:
        game_row = conn.execute('select version_group_id from games where game_id = ?', (game_id,)).fetchone()
        if game_row:
    ...
    rows = conn.execute(
        ...
        + _build_generation_patch_join('species_stats', 'ss', 'sp.species_id', '?')
        + _build_generation_patch_join('species_types', 'st', 'sp.species_id', '?')
        + _build_generation_patch_join('species_abilities', 'sa', 'sp.species_id', '?')
...
def get_pokemon_trainers_and_badges(conn, run_id, attempt_number, pokemon_id):
    ...
    pokemon = conn.execute(
        ...
        f'from pokebank pb '
        f'join attempts a on pb.attempt_id = a.attempt_id '
        f'where pb.pokemon_id = ? and pb.run_id = ? and a.attempt_number = ?',
    ...
    if defeated_trainer_ids:
        placeholders = ','.join('?' * len(defeated_trainer_ids))
